...
exports.build = async ({
  ...
  if (path.basename(entrypoint) !== 'index.php') {
    throw new Error(
      `Entrypoint file name must be the "index.php" of the Drupal instance. Currently it is ${entrypoint}`
    )
  }
  ...
  const buildedFiles = await saveFiles(workPath)
  ...
  // Prepare entrypoints.
  const indexEntry = removeDocumentRoot({ entrypoint, documentRoot })
  let phpEntrypoints = { [indexEntry]: lambda }

  if (exposeInstall) {
    const installEntry = removeDocumentRoot({
      entrypoint: path.join(path.dirname(entrypoint), 'core', 'install.php'),
      documentRoot
    })
    phpEntrypoints[installEntry] = lambda
  }
